...
export default class ExtendedList extends Vue<Props> {
  ...
  get bestPosition(): "top" | "bottom" {
    ...
    const listHeight = isFullyEstimated
      ? this.listHeight
  ...
  private getGroup(id: OptionId): void {
  ...
  public render(): h.JSX.Element {
    ...
    return (
      ...
      >
        {!state.hideFilter && <Filter store={this.store} />}
          ...
          <span
            class={[
              "selectic-item selectic-item--header selectic-item__is-group",
              {
                ...
                selectable:
                  unref(this.store.allowGroupSelection) &&
                  !this.topGroupDisabled,
        ...
        />
        {this.infoMessage && (
          <div class="selectic__message alert-info">{this.infoMessage}</div>
        )}
        {this.searching && (
